...
abstract class Visibility
{
    ...
    /// </param>
    public abstract void Compute(TilePos origin, int rangeLimit);
}
...
sealed class MyVisibility : Visibility
{
    ...
    public override void Compute(TilePos origin, int rangeLimit)
    {
        _setVisible(origin.x, origin.z);
    ...
    void Compute(uint octant, TilePos origin, int rangeLimit, uint x, Slope top, Slope bottom)
    {
    ...
    bool BlocksLight(uint x, uint y, uint octant, TilePos origin)
    {
        uint nx = (uint)origin.x, ny = (uint)origin.z;
        switch (octant)
        {
    ...
    void SetVisible(uint x, uint y, uint octant, TilePos origin)
    {
        uint nx = (uint) origin.x, ny = (uint) origin.z;
        switch (octant)
        {
